...
const official = [
  'go.kr','or.kr','gov.kr','car365.go.kr','cyberts.kr','ev.or.kr','mecar.or.kr',
  'bizinfo.go.kr','sbiz24.kr','bokjiro.go.kr','wetax.go.kr','nts.go.kr','hometax.go.kr',
  'mnuri.kr','energyv.or.kr','law.go.kr','me.go.kr'
];
const banned = [
  '정부 공식 사이트입니다','100% 지급','무조건 지급','승인 보장','공식 지원센터',
  '하나의 콘텐츠를 단계별로 나누어 안내합니다.'
];
const required = [
  'title','description','slug','category','publishedAt','updatedAt','reviewedAt',
  'author','keywords','keyPoints','sources','faqs','body','series','officialCta'
];
...
for (const rel of files) {
  ...
  if (!data.description || data.description.length < 45 || data.description.length > 180) {
    errors.push(`${rel}: description must be 45-180 chars`);
  }
  ...
  if ((data.body || '').length < 3000) errors.push(`${rel}: body too thin (${(data.body || '').length})`);
  ...
  if (!Array.isArray(parts) || parts.length !== 3) {
    ...
    parts.forEach((part, index) => {
      ...
      if (!part.description || part.description.length < 35 || part.description.length > 180) {
        errors.push(`${rel}: ${label} description must be 35-180 chars`);
      }
      if (!Array.isArray(part.keyPoints) || part.keyPoints.length < 3) {
        errors.push(`${rel}: ${label} needs at least 3 key points`);
      }
      if (!part.body || part.body.length < 1000) {
        errors.push(`${rel}: ${label} body too thin (${(part.body || '').length})`);
      }
      if (!part.image || !/^https:\/\//.test(part.image)) {
        errors.push(`${rel}: ${label} HTTPS image required`);
      }
      if (!part.imageAlt || part.imageAlt.length < 10) {
        errors.push(`${rel}: ${label} imageAlt required`);
      }
      if (index < 2 && (!part.nextLabel || part.nextLabel.length < 6)) {
        errors.push(`${rel}: ${label} nextLabel required`);
      }
    });

    const totalSeriesLength = parts.reduce((sum, part) => sum + (part.body || '').length, 0);
    if (totalSeriesLength < 3000) errors.push(`${rel}: series total body too thin (${totalSeriesLength})`);
  }

  if (!data.officialCta?.href || !/^https:\/\//.test(data.officialCta.href)) {
    errors.push(`${rel}: official HTTPS CTA required`);
  }

  const urls = (data.sources || []).map((source) => source.url).filter(Boolean);
  if (!urls.some((url) => {
    try {
      return official.some((host) => new URL(url).hostname.endsWith(host));
    } catch {
      return false;
    }
  })) {
    errors.push(`${rel}: official source required`);
  }
...
console.log(`Content validation passed: ${files.length} article(s), all with 3 rich pages and images.`);
